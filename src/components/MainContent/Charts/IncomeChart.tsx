...
const IncomeChart: React.FC<Props> = ({ financialData }) => {
  ...
  const [tooltipContent, setTooltipContent] = useState<{ credit: number; debit: number; profit: number; loss: number }>({ credit: 0, debit: 0, profit: 0, loss: 0 });
  ...
  const handleMouseMove = (event: MouseEvent) => {
    if (chartRef.current) {
      ...
      const dataPointIndex = chartInstanceRef.current?.scales.x.getValueForPixel(x);
          ...
          <div className="relative z-50">
            ...
            <div>
              ...
              <div className="flex justify-between">
                ...
                <p className="text-xs text-blue-500">${tooltipContent.credit.toFixed(2)}</p>
              </div>
              
              <div className="flex justify-between">
                <p className="text-xs">Debit:</p>
                <p className="text-xs text-yellow-500">${tooltipContent.debit.toFixed(2)}</p>
              </div>
              
              <hr className="h-0.5 bg-slate-400 my-1" />
              <div className="flex justify-between">
                <p className="text-xs">Profit:</p>
                <p className="text-xs text-green-500">${tooltipContent.profit.toFixed(2)}</p>
              </div>
              <div className="flex justify-between">
                <p className="text-xs">Loss:</p>
                <p className="text-xs text-red-500">${tooltipContent.loss.toFixed(2)}</p>
              </div>
            </div>
          </div>
        </div>
